...
figure; plot(t, y); ylim([-1.1 1.1]); title('Sinal banda base polar NRZ')
...
while i < length(y)
     if isequal(y(i:i+7), seq)
        ...
        tamanho_info_decimal = bi2de(y(i+10:i+24));

        % Armazena os próximos "tamanho_info" bits em "rx_bytes" (em binário)
        rx_bits{id2} = y(i+25:i+25+tamanho_info_decimal);
        % Armazena os próximos 32 bits em "vetcrc" (em binário)
        vetcrc{id2} = y(i+25+tamanho_info_decimal:i+57+tamanho_info_decimal);
...
for id = 1:3
    ...
    rx_decoded = huffmandeco(rx_bits{id}, dict);
    
    rx_bytes(id,1:end-2) = rx_decoded(1:size(rx_bytes,2)-2);
